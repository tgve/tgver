...
Without bundling the edges
```{r plot-no-bundling, out.width='90%'}
plot(sfc)
```

Using the `pbundle`, please see the `edgebundle` package for details of the algorithm, and using `ggplot`:
...
sfc = lapply(1:100, function(x) st_linestring(matrix(unlist(pbundle[pbundle$group == x, 1:2]), ncol = 2)))
sfc = st_sfc(sfc, crs = 4326)
# plot(sfc)
